        ...
        <div className="flex flex-col gap-5">
          <div className="grid grid-cols-2 gap-2">
            {JOURNEY_STEPS.map(s => (
              <label
                key={s.value}
                className={`flex items-center gap-2 px-3 py-2 border rounded-sm cursor-pointer text-sm font-lora transition-colors ${status === s.value ? 'border-teal bg-teal/5 text-teal' : 'border-[#e5e5e0] text-ink/70 hover:border-teal/30'}`}
              >
          ...
          <textarea
            className="w-full h-24 px-3 py-2.5 bg-white border border-[#e5e5e0] rounded-sm font-lora text-sm text-ink placeholder:text-ink/40 focus:outline-none focus:ring-1 focus:ring-teal resize-y"
